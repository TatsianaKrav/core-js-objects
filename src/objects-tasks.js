...
function mergeObjects(objects) {
  return objects.reduce((acc, obj) => {
    ...
    Object.entries(obj).forEach(([key, value]) => {
      copyAcc[key] = copyAcc[key] ? copyAcc[key] + value : value;
    });
    return copyAcc;
  }, {});
}
...
function makeWord(lettersObject) {
  const arrOfIndexes = Object.values(lettersObject).flat(Infinity);

  Object.entries(lettersObject).forEach((obj) => {
    const [key, value] = obj;

    value.forEach((val) => {
      arrOfIndexes[val] = key;
    });
  });

  return arrOfIndexes.join('');
}
