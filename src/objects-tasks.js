...
function shallowCopy(obj) {
  let shallowCopyObj = {};
  shallowCopyObj = Object.assign(shallowCopyObj, obj);
...
function removeProperties(obj, keys) {
  ...
  Object.keys(obj).forEach((key) => {
    const sameValue = keys.find((item) => item === key);
    if (sameValue) delete copyObj[key];
  });
...
function makeWord(lettersObject) {
  ...
  Object.values(lettersObject).forEach((arr) => {
...
function group(array, keySelector, valueSelector) {
  const map = array.reduce((obj, item) => {
    ...
    newObj[key].push(valueSelector(item));
    return newObj;
  }, {});
